<?	
	...
	$sql = "SELECT gal_total_fotos FROM galeria WHERE gal_codigo = ".$gal;
	$rs = @mysql_query($sql);
	...
	if($ini < $total){				
		$fotos = array();	
		$diretorio = "../imagens/galerias/".$gal."/pequenas/";
		$i == 0;
...
?>
<script>
var atual = '<? echo '../imagens/galerias/'.$gal.'/grandes/1.jpg' ?>';
	...
	<table width="442" height="181" border="0" align="center" class="noticia">
  	...
  	<tr>		
        ...
        <?
						if($int > 1){
							for($i = 0; $i < $int; $i++){
								echo "<a class='links' href='index.php?id=7&gal=".$gal."&pag=".($i + 1)."'>".($i + 1)."</a>";
								if($i != $int - 1){
									echo " &bull; ";
								}
							}
  	...
  	<tr>
    	<td colspan="5" align="center" valign="top">
				<iframe src="<? echo 'img.php?foto=../imagens/galerias/'.$gal.'/grandes/1.jpg'; ?>" frameborder="0" scrolling="no" name="foto" height="327" width="442"></iframe>			</td>
	  </tr>    
	</table>
<?
	}else{
