<?
	...
	$sql = "SELECT not_titulo,not_data,not_codigo FROM noticias ORDER BY not_data DESC,not_hora DESC LIMIT $min,$limite";
	$not = mysql_query($sql);
?>
<script>
function excluir(cod){
	if(confirm('Deseja realmente excluir esta notícia?')){
		location = "scr_noticias.php?op=excluir&cod=" + cod;
	}
}
</script>
<table width="598" height="66" border="0" align="left" cellpadding="0" cellspacing="0" class="noticia">
  <tr>
    <td height="22" colspan="3" valign="top" bgcolor="#EEEEEE" class="bordaBaixo"><div class="topicos">Lista de not&iacute;cias&nbsp;</div> </td>
  </tr>	
  <tr>
  ...
  <tr>
    ...
    <td align="justify" valign="middle" class="datNoticia2"><a target="_blank" href="../php/index.php?id=1&cod=<? echo $nt[2]; ?>" >
      <? 
					if(strlen($nt[0]) > 60){
    ...
    <td align="center" valign="middle" class="datNoticia2">
			<a class="links" href="index.php?id=8&cod=<? echo $nt[2]; ?>">
				<img src="../img/exchange.png" border="0" alt="Editar" title="Editar" />
			</a>
			<a class="links" href="javascript:excluir(<? echo $nt[2]; ?>);">
				<img src="../img/delete.png" border="0" alt="Excluir" title="Excluir" />
			</a>
		</td>
  </tr>    
  <?
